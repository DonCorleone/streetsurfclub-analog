...
export class BloggerService {
  ...
  getPage(pageid: string): Observable<Page | null> {
    return this.httpClient.get<Page>(`${this.apiBaseUrl}/get-page?pageid=${pageid}`).pipe(
      catchError(err => {
  ...
  getPost(postid: string): Observable<Post | null> {
    ...
    return this.httpClient.get<Post>(`${this.apiBaseUrl}/get-post?postid=${postid}`).pipe(
      map(post => {
        if (post) {
